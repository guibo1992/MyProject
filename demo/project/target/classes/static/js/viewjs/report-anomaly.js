...
$(function() {
	...
	var $table = $('#table'), $form = $('#anomalyForm'), $modal = $('#anomalyModal');
	var $type = $('#type');
	...
	$form.formValidation({
		...
		$(e.target).ajaxSubmit({
        	url: '/report/anomaly/add',
        	type: 'post',
        	dataType: 'json',
        	beforeSubmit: function(arr, $form, options) {
        		if ( $form.find('[name="id"]').val() ) {
        			options.url = '/report/anomaly/' + $form.find('[name="id"]').val() + '/update';
        		}
        	},
	...
	window.operateEvents = {
		'click .edit' : function(e, value, row, index) {
			...
			$.get('/report/anomaly/' + row.id, function(data) {
				if (!data.success) {
					$.alert({
						title : '错误',
						content : data.message,
				...
				var anomaly = data.data;
				$form.find('.form-control').val('');
				$form.find('[name="id"]').val(anomaly.id);
				$form.find('[name="typeId"]').val(anomaly.typeId).trigger('chosen:updated');
				$form.find('[name="anomaly"]').val(anomaly.anomaly).trigger('keyup');
				$form.formValidation('resetForm');
				$modal.modal('show');
			});
		},
			...
			$.confirm({
				...
				buttons: {
					confirm: {
		                ...
		                btnClass: 'btn-danger',
		                action: function () {
		                	$.get('/report/anomaly/' + row.id + '/delete', function(data) {
								if (!data.success) {
									$.alert({ 
										title: '错误',
										content : data.message,
