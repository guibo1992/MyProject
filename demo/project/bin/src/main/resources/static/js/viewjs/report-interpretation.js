...
$(function() {
	...
	var $table = $('#table'), $form = $('#interpretForm'), $modal = $('#interpretModal');
	var $type = $('#type');
	...
	$form.formValidation({
		...
		$(e.target).ajaxSubmit({
        	url: '/report/interpretation/add',
        	type: 'post',
        	dataType: 'json',
        	beforeSubmit: function(arr, $form, options) {
        		if ( $form.find('[name="id"]').val() ) {
        			options.url = '/report/interpretation/' + $form.find('[name="id"]').val() + '/update';
        		}
        	},
	...
	function operateFormatter(value, row, index) {
		var array = new Array();
		array.push('<a class="edit btn btn-primary margin-r-5 margin-btm-5" href="#" data-placement="left" title="编辑结果解释">');
		array.push('<i class="fa fa-edit"></i> 编辑');
		array.push('</a> ');
	
		array.push('<a class="delete btn btn-danger margin-btm-5" href="#" data-placement="left" title="删除结果解释">');
	...
	window.operateEvents = {
		'click .edit' : function(e, value, row, index) {
			...
			$.get('/report/interpretation/' + row.id, function(data) {
				if (!data.success) {
					$.alert({
						title : '错误',
						content : data.message,
				...
				var interpret = data.data;
				$form.find('.form-control').val('');
				$form.find('[name="id"]').val(interpret.id);
				$form.find('[name="typeId"]').val(interpret.typeId).trigger('chosen:updated');
				$form.find('[name="interpretation"]').val(interpret.interpretation).trigger('keyup');
				$form.formValidation('resetForm');
				$modal.modal('show');
			});
		},
			...
			$.confirm({
				...
				buttons: {
					confirm: {
		                ...
		                btnClass: 'btn-danger',
		                action: function () {
		                	$.get('/report/interpretation/' + row.id + '/delete', function(data) {
								if (!data.success) {
									$.alert({ 
										title: '错误',
										content : data.message,
