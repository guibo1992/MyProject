...
function createCanvas(opts) {
  ...
  var ts = opts.b.split('\n');
  var y = - ((ts.length - 1) / 2 ) * (opts.l * opts.g);
  
  for(var i = 0; i < ts.length; i++){
  	cc.fillText(ts[i], 0, y);
  	y += opts.l * opts.g;
  }
...
function prepareOpts(opts) {
  ...
  fsize = opts.fsize || '20', // auto calculate the font size to response to the image size
  ffamily = opts.ffamily || 'consolas', // 规定字号和行高，以像素计。
  lheight = opts.lheight || 1.3,
  new_opts = {};
