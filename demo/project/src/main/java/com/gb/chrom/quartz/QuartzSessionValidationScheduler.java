...
//重写SessionValidationScheduler
/**
 * 基于Quartz 2.*
 *
 * @author Summer Created at 2018年3月20日
 * @since 1.0.1
 */
	...
	public void enableSessionValidation() {
		...
		try {
			SimpleTrigger trigger = TriggerBuilder.newTrigger().startNow().withIdentity(JOB_NAME, Scheduler.DEFAULT_GROUP)
					.withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInMilliseconds(sessionValidationInterval)).build();// <span style="color:#ff0000;">Quartz
																																		// 2中的实现</span>
